...
class CondaDeployment(AbstractDeployment):
    ...
    async def start(self):
        # If instructed to reuse host env, skip creating a new conda env
        if os.getenv("SWEAGENT_USE_HOST_ENV", "0").lower() not in ("", "0", "false"):
            self.logger.info("SWEAGENT_USE_HOST_ENV enabled; reusing current environment and skipping conda creation.")
            self._env_prefix = Path(sys.prefix)
            self._write_rcfile(skip_conda=True)
    ...
    def _write_rcfile(self, skip_conda: bool = False):
        prefix = str(self._env_prefix)
        ...
        lines = [
            ...
            "fi",
            "",
            f'export PIP_CACHE_DIR="{self._pip_cache}"',
            ...
            "",
            f'export SWE_TOOLS_ROOT="{tools_root}"',
            'export PATH="$SWE_TOOLS_ROOT/bin:$PATH"',
        ]
        if skip_conda:
            lines.append(f'# Using host environment, no conda activation\nexport CONDA_PREFIX="{prefix}"')
        else:
            lines.append(f'conda activate "{prefix}"')
